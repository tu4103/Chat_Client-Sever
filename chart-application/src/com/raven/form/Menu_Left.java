...
//        }
//        refreshMenuList();
    	 List<Model_User_Account> groupUsers = new ArrayList<>();

    	    // Thêm các người dùng vào danh sách groupUsers. Đây là một ví dụ, bạn cần điều chỉnh nó để phù hợp với dự án của bạn.
    	    groupUsers.add(userAccount.get(0));
    	    groupUsers.add(userAccount.get(1));
    	    groupUsers.add(userAccount.get(2));

    	    // Xóa tất cả các thành phần hiển thị trong menuList
    	    menuList.removeAll();

    	    // Hiển thị danh sách người dùng trong group
    	    for (Model_User_Account user : groupUsers) {
